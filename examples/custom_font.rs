...
impl State {
    fn new(window: Arc<Window>) -> Self {
        ...
        // Load custom font
        let family_name = text.load_font(include_bytes!("../examples/PublicPixel.ttf")).expect("Failed to load font");
        
        let custom_style = text.add_style(TextStyle {
            font_size: 32.0,
        ...
        let text_edit = text.add_text_edit("Text edit box with custom font.\nFont fallback: 無限での座を含む全ての".to_string(), (50.0, 50.0), (600.0, 300.0), 0.0);
        text.get_text_edit_mut(&text_edit).set_style(&custom_style);
        ...
        if self.state.is_none() {
            let window = Arc::new(event_loop.create_window(
                Window::default_attributes()
                    .with_title("Custom Font Example")
